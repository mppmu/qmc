#ifndef QMC_FITFUNCTIONS_POLYSINGULARONESIDED_H
#define QMC_FITFUNCTIONS_POLYSINGULARONESIDED_H
...
namespace integrators
{
    ...
    {
        template <typename D>
        struct PolySingularOneSidedFunction
        {
        ...
        template <typename D>
        struct PolySingularOneSidedJacobian
        {
            static const int num_parameters = 4;
        ...
        template <typename D>
        struct PolySingularOneSidedHessian
        {
        ...
        struct PolySingularOneSidedTransform
        {
        ...
        template<typename I, typename D, U maxdim>
        struct PolySingularOneSidedImpl
        {
            using function_t = PolySingularOneSidedFunction<D>;
            using jacobian_t = PolySingularOneSidedJacobian<D>; // set to std::nullptr_t to compute numerically
            using hessian_t = PolySingularOneSidedHessian<D>; // set to std::nullptr_t to compute numerically (also set fitparametersgsl.trs = gsl_multifit_nlinear_trs_lm);
            using transform_t = PolySingularOneSidedTransform<I,D,maxdim>;
        };
        
        struct PolySingularOneSided
        {
            template<typename I, typename D, U maxdim> using type = PolySingularOneSidedImpl<I, D, maxdim>;
        };
